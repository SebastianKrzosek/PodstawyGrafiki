...
{
    if(wybor == 6 || wybor ==7 ){ //do rysowania krzywej
        startX = event->x();
        startY = event->y();
        startX -= poczX;
        startY -= poczY;
...
void MyWindow::rysujKrzywa(){
    double x,y,t3,t2,a,b,c,d;
    //double x,y;
    for(int i=0 ; i<ilePunktow-3; i++){
        for(double t = 0; t<=1; t+=0.0001)
        {
            ...
            c = (-3*t3+3*t2+3*t+1)/6;
            d = t3/6;
            x= punktyX[i+0]*a + punktyX[i+1]*b + punktyX[i+2]*c + punktyX[i+3]*d;
            y= punktyY[i+0]*a + punktyY[i+1]*b + punktyY[i+2]*c + punktyY[i+3]*d;
            rysujPiksel(round(x),round(y));
        }
    }
}
...
void MyWindow::on_radioButton_5_clicked() //bspline
{
    wybor = 6;
}

void MyWindow::on_radioButton_6_clicked() //przesuwanie
{
    wybor = 7;
}
